...
from app.repository.getter import get_by_short_code
from app.utils.code_generator import CodeGenerator

def delete_by_short_code(session_factory, code: str) -> URL | None:
    """Delete a URL record by short code if it exists, using get_by_short_code and decode for ID.

    Returns:
        URL: The deleted URL object if found and deleted, None if not found.
    """
    url_obj = get_by_short_code(session_factory, code)
